...
internal_x86_code_v3_provable_prefilter_unrolled:
    ...
    mov rbp, rsp
    /* rdi = buffer base (p)
       This unrolled block checks offsets 0..31. For each offset i:
         b = p[i]
         if (b == 0xE8 || b == 0xE9) then
             ms = p[i+4]
             if (ms != 0 && ms != 0xFF) return i
    */
    /* default: none found -> return 32 */
    mov rax, 32
...
.NEXT27:
    ...
    jmp .NEXT28
.CHK28:
    /* p[i+4] would be at rdi+32 which is out of block; treat as not matching */
    jmp .NEXT28
.NEXT28:
